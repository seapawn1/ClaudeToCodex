...
状态：仍仅实施 PBI-05；按 PO 更正，交付渠道为公开仓库 `seapawn1/ClaudeToCodex` 的 GitHub Releases。第一、二节已同步；下一步由 Developer 完成候选上传与下载核对。
...
首批用户为 PO 本人，本次只交付基于现有能力的最小版本包，通过 GitHub Releases 分发，PO 从 GitHub 下载后安装和验收。
...
PBI-05 不再拆成多个 PBI。只做「版本包与 GitHub 分发、最短说明、从 GitHub 下载验收」三部分，复用现有 bridge 和安装入口。

| 内容 | 本次精化 | 验收标准 |
|---|---|---|
| 版本包 | GitHub Release 提供版本 ZIP、整包校验值与版本说明，包内含文件清单；正式版本对应 Git tag `v1.0.0`。本地目录只作为构建与暂存位置。 | PO 能从目标 GitHub 仓库的 Release 下载明确标识版本的完整包，核对来源版本、文件清单与校验值；包中不夹带本机会话端点、令牌、配对或消息记录。 |
| 最短安装使用说明 | 从 GitHub Release 下载资产开始，复用现有安装命令，在无现成 bridge 配置的新项目启用；说明前置条件、解压位置、安装、必要信任与重载、Claude 侧入站策略确认、登记、配对及发送回复，沿用当前已声明的能力边界。 | PO 从 GitHub 取得包并在仓库之外解压，仅按包内说明即可安装并使用；产品入口及 hook 路径指向解压后的产品文件，不依赖开发工作区。 |
| 一轮 PO 验收 | 先将候选资产上传到 GitHub 草稿 Release，由有仓库访问权限的 PO 从 GitHub 下载，在上述新项目中完成一次真实协作往返；复用适用的现有离线回归。 | 适用回归通过，GitHub 下载资产与候选包校验值一致；两个原始会话完成请求、回复、追问与再答，以唯一标记和接收方原始会话事件核对，反馈推进实际工作且 PO 无需转述；记录版本、环境、问题与 PO 结论。验收通过后正式发布同一份资产，内容不重建、不替换。 |
