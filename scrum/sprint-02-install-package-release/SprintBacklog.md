...
- 2026-09-09：依 PO-ASSISTED-ACCEPTANCE-AR 制定候选 2 现场验收运行手册（scrum/sprint-02-install-package-release/PO-LIVE-ACCEPTANCE-RUNBOOK.md）：环境隔离=全新项目目录（避开 Planning 旧 hooks）+操作员预设隔离数据目录 bridge-accept-c2（PO 零环境配置）+PO 真实 CODEX_HOME（避开未登录障碍）；PO 仅做登录（如需）、/hooks 信任三条 hook、启动两侧会话、自然语言交流（含唯一标记）；操作员负责环境准备、安装缓存与候选 2 ZIP 逐文件绑定核对、events.jsonl 的 context-prepared 与 skill 会话记录采集、仓库复判（两证齐全预期 10/0/0 退出 0）。
- 2026-09-09：依 RUNBOOK-PREPARATION-AS 交付 PO 启动入口 scrum/sprint-02-install-package-release/po-acceptance-c2/Start-AcceptCodex.ps1（校验插件已装→进入 D:\ClaudeToCodex-Accept→仅本窗口设定 bridge-accept-c2→启动 Codex，支持 codex resume 选择器继续验收会话；PO 零手配）并修正手册两处口径：①「不是产品缺陷」改为证据中性表述（未观察到实际运行结果，不断言有无缺陷）；②注明操作员覆盖数据目录属验收隔离、不单独作为默认目录零配置证据，context-prepared 须与真实宿主执行事件、原始收信会话、唯一标记三方绑定。未自行安装或启动（SM 负责环境准备）；候选 2 字节不变。
- 2026-09-09：SM 完成操作员准备（D:\ClaudeToCodex-Accept 与 bridge-accept-c2 建好无 pair；真实用户缓存自 GitHub 固定 b6286c0 安装候选 2、19 文件与 ZIP 一致；Planning pair 与项目 hooks 哈希前后不变、hooks 未信任；记录 po-acceptance-c2/environment-preparation.json）。依 PO-PREPARED-AT 落实登录口径：实测 codex login status 未确认已登录——启动入口横幅与手册改为「按 CLI 实际提示自行登录」，不再写「本机已登录/通常已登录」。入口与手册修正提交待推。
