...
| S04-11-4 真实交流 | 通过（真实，B 追问闭环补跑中） | A 两轮、B 一轮+追问，全部自动唤醒回链（wake-submitted→context-prepared）；1f8a1145 无入站（无误投） |
| S04-11-5 重叠来信 | 通过（真实） | 18:31:46.183（B 发布）至 18:31:51.701（A 领取/上下文准备）为发布→领取间隔，期间两槽并存（精确并存时长以 claim 边界/槽快照为准，此为间隔下界证据）；双 wake（44.352/46.701）先于任一投递入队；A 经 UserPromptSubmit、B 经 PostToolUse 逐事件进入，无覆盖无串目标；B 重复 wake 被抑制各一次；1f8a1145 同槽拒绝=未接纳。d65bd878 为人工恢复样本，另册不计自动唤醒 |
...
| 工作项 | 内容 | 估计 | 状态 |
|---|---|---|---|
| W6 | 真实单目标生命周期隔离（S04-11-6）：停 A 验 B 全程可用；旧配对发送→端点失效如实 send-error 不误投；A 重启 connect 成新配对；同名歧义如实报错；补最小显式 retire 操作作为重建边界（仅原型）；旧消息回复不进新原会话 | 0.75d | 未开始 |
| W6b | CLI 异常分支真实层（补 S04-11-2）：未知目标/歧义目标/失效回复目标，可观察报错与下一步 | 0.25d | 未开始 |
| W7 | 单配对真实回归 + 旧数据路径（S04-11-7）：单目标完整往返不回退；legacy pair.json 只读识别、同身份沿用、跨身份共存的真实验证 | 0.5d | 未开始 |
