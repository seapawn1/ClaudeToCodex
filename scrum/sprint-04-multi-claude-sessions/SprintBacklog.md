...
| S04-11-2 目标确定 | 部分：真实投递正确；歧义/失效分支未测 | A×3、B×3 均投正确原会话（入站帧核对）；不存在/歧义分支仅 fixture T3 |
...
| S04-11-6 单目标生命周期隔离 | 未测 | run1/2 会话重启素材已记 DESIGN §6 缺口（同名歧义无退役路径）；按 AC 的停 A 保 B 场景未执行 |
| S04-11-7 单目标兼容 | 部分（fixture T10） | 未做专门的单配对真实回归与旧数据路径 |
...
| W6b | CLI 异常分支真实层（补 S04-11-2）：未知目标/歧义目标/失效回复目标，可观察报错与下一步 | 0.25d | 部分完成：同名歧义与失效端点已在 W6 真实层覆盖；未知目标/未知回复目标的真实层分支未跑 |
| W7 | 单配对真实回归 + 旧数据路径（S04-11-7）：单目标完整往返不回退；legacy pair.json 只读识别、同身份沿用、跨身份共存的真实验证 | 0.5d | 进行中：代码层已恢复 1.0.0 单目标 `send` 免 `--name`（恰一目标沿用，零/多目标显式报错；CLI 级测试覆盖）；真实层单配对往返与 legacy 数据路径待跑 |
