...
| S04-11-7 单目标兼容 | 通过（真实；原型的历史通过对最终候选仍需按候选层复验） | 单目标免 --name 真实往返（77b5fb5d→d7392a3a）✓。legacy 真实闭环（W10，SM 已核三帧入站）：搁浅回复 de5319b9 恢复投递（单列恢复样本）→ 端点原位刷新 → 免 --name 新消息（fb2dce20）→ B 干净自动闭环（7ac38f36）✓。**工作中收信双边界（W11）**：T03 模型生成中——B 信 19:52:07 发布/wake 19:52:09，生成与落盘完成边界 19:52:18 投递，短文完整未截断；T04 工具执行中——B 信 19:53:25 发布，19:53:30 投递，会话自记睡眠中收到、睡眠未中断；另有一次真实重复 wake 抑制（wake-suppressed 47c5824e 19:53:00）。八-1 未构成样本已如实保留。注：期间一次窗口重启误用新线程启动器（01a09723 的 connect 被单根单 Codex 边界如实拒绝——该边界本身的意外真实层证据），已改 resume 启动器回原线程 |
