...
| W6b | CLI 异常分支真实层（补 S04-11-2）：未知目标/歧义目标/失效回复目标，可观察报错与下一步 | 0.25d | 2026-09-13 完成：同名歧义（W6 两次）、失效端点（a6fd3f8a/7b13c457）、未知目标（No connected target matches 并列已知目标）、未知回复（ENOENT）均有真实层证据，落 lifecycle-log |
