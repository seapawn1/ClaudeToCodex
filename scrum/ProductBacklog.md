...
| PBI-11 | 一 Codex 对多 Claude Code 会话 | 作为日常使用者，我要让一个 Codex 会话同时与多个 Claude Code 会话保持桥接并按名称往来，以便并行开展多项协作，而不是每次换目标都要换配对。 | 现行桥为单对模型：单数据目录、单 pair.json、发送方身份校验限单会话；多会话需多配对共存、按会话名称寻址与路由、状态与换配对边界相应扩展；具体架构待后续研究精化。 | Sprint 04 方向（PO 确认；尚未启动） | PO 于 2026-09-11 确认保留 Sprint 04 方向；当前尚未启动，团队承诺待 Developer 与容量确认。最小切片为一个 Codex 原始会话与至少两个按名称选择的 Claude 原始会话独立收发，保留显式目标、证据和归档边界；不默认承诺自动切换、广播、并发投递或任意规模。验收标准待后续研究与 PO 明确。 |
| PBI-12 | 多会话协作中的目标辨识 | 作为同时管理多个 Claude Code 会话的日常使用者，我要在发送前清楚知道消息将进入哪个原始会话，并能发现目标选择错误，以免把消息发给错误的会话。 | 多会话协作中的目标选择、项目上下文辨识与误投风险；具体交互和技术方案待研究，不预设确认按钮或其他实现方式。 | 待精化 | 来自 Sprint-04 Planning 暴露的真实误投风险。本条先记录重要用户故事与问题边界，不纳入当前 Sprint；验收标准待研究后与 PO 明确，需验证用户能辨识目标、发现错误选择并避免消息误投，同时保持原始会话与关联证据规则。 |

| PBI-13 | 插件连接状态与生命周期操作 | 作为日常使用者，我要能快速检查插件与桥接连接的当前状态，并在需要时明确结束、断开或处理连接，以便不必翻查数据目录或依赖记忆排查日常连接问题。 | 插件连接状态、连接生命周期和用户可操作边界待研究；具体命令、交互和数据保留策略不预设，须与现有状态、配对、端点失效和归档规则保持一致。 | 待精化 | 来自当前使用反馈：现有入口主要依赖单一 skill，用户缺少快捷的状态检查与连接操作路径。本条先记录重要用户故事，不纳入当前 Sprint；验收标准待研究后与 PO 明确，需覆盖状态可理解、操作边界明确、断开不会静默丢失历史证据，并能通过标准产品入口验证。 |
