...
| PBI-11 | 一 Codex 对多 Claude Code 会话 | 作为日常使用者，我要让一个 Codex 会话同时与多个 Claude Code 会话保持桥接并按名称往来，以便并行开展多项协作，而不是每次换目标都要换配对。 | 现行桥为单对模型：单数据目录、单 pair.json、发送方身份校验限单会话；多会话需多配对共存、按会话名称寻址与路由、状态与换配对边界相应扩展；具体架构在 Sprint 04 pre-study 中精化。 | Sprint 04 选中（PO 方向；团队承诺待容量确认） | PO 于 2026-09-11 将本条定为 Sprint 04 唯一方向项，团队承诺待容量确认。最小切片为一个 Codex 原始会话与至少两个按名称选择的 Claude 原始会话独立收发，保留显式目标、证据和归档边界；不默认承诺自动切换、广播、并发投递或任意规模。验收标准详见 scrum/sprint-04-multi-claude-sessions/SprintBacklog.md。 |

| PBI-12 | 多会话发送前目标确认与误投防护 | 作为同时管理多个 Claude Code 会话的日常使用者，我要在发送或切换目标前清楚看到并确认目标会话及其项目上下文，以免把消息发给错误的原始会话。 | 在多会话路由上增加可理解的目标预览与显式确认边界；确认形式（CLI 交互提示、显式参数或其他入口）待精化，不预设为按钮；与按名称寻址、状态和换配对规则保持一致，不静默切换或替换。 | 待精化 | 来自 Sprint-04 Planning 暴露的真实误投风险。先记录问题与用户故事，不纳入当前 Sprint；验收标准待精化后与 PO 明确，需覆盖目标名称/项目上下文可辨识、切换时显式确认、取消或拒绝后不发送、确认后仍按原始会话和关联字段核对送达。 |
