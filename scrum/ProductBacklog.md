...
|---|---|---|---|---|---|
| PBI-01 | 产品化已验证的跨会话双向消息桥 | 作为 PO，我要在 Codex 与 Claude Code 的现有会话之间直接发送和接收工作消息，以便无需搬运上下文即可推进协作。 | 将 Design Sprint 已验证的最小原生双向桥产品化；不扩展到重启恢复、并发、多会话、远程协作等未验证范围。 | 已交付，PO 最终检查通过（含偏差记录） | 首个 Increment 聚焦已验证的短消息双向通信路径；Review / Retro 与最终检视摘要见 [Sprint 01 Bridge Review & Retro](../docs/scrum-sprint/sprint-01-bridge-review-retro.md)。 |
| PBI-02 | 使用说明与排查修正 | 作为 PO，我要在新用户按说明配置和恢复时避免两个实测踩坑与身份边界误解，以便降低使用成本。 | 文档修正：补充双侧数据目录不一致、端点失效重配对排查；澄清环境变量仅用于路由匹配；区分 Node 最低前置与已验证基线。 | 待精化 | 来自 Sprint 01 Review 的低成本高收益候选。 |
| PBI-03 | 投递失败状态透明化 | 作为 PO，我要区分尝试投递、失败投递与已被接收方领取的消息，以免排查时被本地记录误导。 | 改进 send-error、status 与 messages 存储的可解释性；不引入自动重试或事务性投递。 | 待精化 | Sprint 01 最终检视暴露的唯一产品缺陷候选。 |
| PBI-04 | 产品成效度量定义 | 作为 PO，我要用可观察证据判断桥是否真的减少消息搬运并改善协作，而非只验证技术通道。 | 与 PO 定义 Definition of Outcome Done、真实协作事件样本和检视方式。 | 待精化 | Sprint 02 Planning 的重要输入。 |
