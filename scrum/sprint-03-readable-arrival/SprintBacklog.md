...
- 状态：Sprint Planning 进行中；范围已拆分，Developer 须将 v2.3 HOW 收敛为本 Sprint 范围并回报审阅。
...
### 基本 HOW（待 Developer 按 Sprint 03 范围同步，v0 转移稿）

- **I0 环境与基线**：Node v24.14.0、codex-cli 0.154.0、Claude Code 2.1.268；源基线 `15309b2`，Planning 基线 `a25860f`。日常宿主与隔离候选分别记录，未知不推定。
- **I2 可读到达**：排队唤醒文本改为可读消息，包含可读头行、完整正文和可执行回复入口；机器标记保留可解析位置。同步提示文案、旧格式兼容与防重复回归。粗估 1–2 天。
- **I2b 空闲唤醒修复**：按收据 turnId 区分“同轮已被另一 handler 服务”与“跨轮真重复唤醒”；前者放行并留下可观测 noop 事件，后者维持抑制。包含同轮双 hook 回归。粗估 0.5–1 天。
- **I6 范围化同步**：仅同步与 PBI-09 相关的双树、README / INSTALL / USAGE / SMOKE / skill 和回归；不提前实现 Sprint 06 / 07 的状态或辨识交互。
- **I7 隔离安装候选验证**：完整清单、metadata、有效 hooks、reply 入口来源、双 hook 形态与原始会话证据。
- **I8 PO 手动 E2E**：真实请求、回复、追问、再答；检视到达即内容可读。PO 参与不计入 Developer 净工作量。
- Developer 拆分评估：09 独立交付约 3.5–6 净天，首个可检视成果为 I2 + I2b 后的一次真实空闲到达。该估算为讨论输入，最终容量由 Developer 在范围内 HOW 中确认。

### 工作项与进展

- 待 Developer 同步 HOW、审阅前两部分并回报。
