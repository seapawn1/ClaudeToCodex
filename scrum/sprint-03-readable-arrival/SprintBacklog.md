...
- 2026-09-15（I7 本地核对）：以 `0e7765d` 构建插件候选（Build-Release.ps1，plugin 模式）。产物 `claude-to-codex-plugin-1.2.0.zip`，SHA256 `37e453aa6a0944ea1ddd247714355d97b68ae71750e5ba191eac16a0bd66ef55`；manifest 记录 sourceCommit `0e7765db4919c3615b96477890a9cf7c7592eb68`、25 文件。核对：store.mjs 包内 SHA256 `a409f4d4…` 与源树一致；hooks.json 三条（UserPromptSubmit/PostToolUse/Stop）均指向 `${PLUGIN_ROOT}/bridge/cli.mjs`；entry.mjs `commandString()` 运行时自安装位置派生（回复入口随安装位置更新）；I2/I2b 代码在包内逐点在位。待办：隔离 home 安装与 S03 系现场格（与 SM 协调）。
