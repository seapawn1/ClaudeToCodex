...
- **全局会话-root 索引**：`%LOCALAPPDATA%\ClaudeToCodex\bridge-roots\` 目录、每会话一个 JSON 文件（临时文件＋独占硬链接：原子发布且并发独占），记录 codexThreadId → 数据根映射；只登记不改绑（身份边界），多会话并发首连各写各的文件、无读改写丢失窗口、崩溃只留被忽略的临时孤儿（S05-SM-REVIEW-06/08 后定稿此形态）。
...
- **2026-09-13 SM 复核阻断项修复（S05-SM-REVIEW-06，e594f9b 修复＋5ccefb4 双树同步＋2fe18fc 环境约束记录）：索引由单文件 read-modify-atomic-replace 改为 `bridge-roots\` 目录、每会话一个 JSON 文件、独占创建（wx）。** 沿用 pairs 注册表同款"每身份一 wx 文件"模式：不同会话并发首连写不同文件，结构性消除丢失更新；同会话并发竞争 wx 落败方校验胜者条目（幂等或拒改绑；胜者短暂未落盘时至多重读 100ms）；不可读条目读取时跳过、bind 拒绝自动覆盖（要求人工修复）。`CTC_ROOTS_FILE` 改名 `CTC_ROOTS_DIR`（本轮未发布，无迁移负担；真实机器无既有索引文件）。新增回归 4 项：API 级双会话共存契约、真实并发进程 8 会话各自绑定零丢失、真实并发进程同会话 8 进程收敛单绑定、不可读条目不覆盖；全套 **79/79 × 双树**。候选重建：SHA256 `a66228e4ff1594a5fdd46f51aa5f9e7f9b1cbaa8733c9158ec22fe4260435664`，源提交 2fe18fc，manifest 25 文件 VERIFY=OK checked=24 extra=0，包级验收 pass=8 fail=0 blocked=3（同前，真实宿主轮归 SM/PO）。
- **2026-09-13 PO 测试环境硬约束纳入（S05-TEST-ENV-07，提交 2fe18fc）**：测试用 Codex 会话新建/恢复命令必须追加 `--profile glm`；测试用 Claude 会话必须在 `D:\ClaudeToCodex` 或其 Sprint worktree 目录启动，不得在临时目录或无关项目启动。已写入 SMOKE 前置表第 8 行与 4c 节，W7 验收编排按此执行。
- **2026-09-13 SM 复核三项修复（S05-SM-REVIEW-08，提交 e7ba853）：** 1) SMOKE R1 改 `bridge-roots\<threadId>.json`（此前漏改的双树遗留）；2) 双树同步范围扩至 release 脚本——`bridge/release/Build-Release.ps1`、`Test-Acceptance.ps1` 与 plugins 树逐字节一致（含 git 仓库根发现与 S05-15 验收桶），双树共享文件集现为 20 个全一致；3) 索引发布改为**临时文件＋独占硬链接**（temp 写入 → `linkSync`，EEXIST 即并发胜者已落位）——最终文件要么完整出现要么不出现，崩溃只留被读取方忽略、bind 不覆盖的临时孤儿，原子性与并发独占同时成立（采纳 SM"原子最终发布"倾向）。新增测试：成功绑定后目录恰一文件无临时残留；预置崩溃临时孤儿不阻塞重绑、孤儿不被清理且我方无残留。全套 **81/81 × 双树**。候选重建：SHA256 `8b97803af295926409195703b05d7d10796bce80219fa606327eeb43dd98afa0`，源提交 e7ba853，manifest 25 文件 VERIFY=OK checked=24 extra=0，包级验收 pass=8 fail=0 blocked=3。
- **2026-09-13 W7 隔离宿主安装事故与恢复（S05-SM-HOST-INSTALL-09 期间，已回报 SM：9f5ef8be）：** Developer 在隔离安装首步误用 PowerShell 只读自动变量 `$home` 赋值（静默失败），`CODEX_HOME` 未生效即执行 marketplace add（本地路径源）与 plugin add——cache 落在 `C:\Users\DELL\plugins\`（惰性杂散），注册写入 `.codex` 共享状态；随后误判并执行 plugin remove / marketplace remove，**删除并注销了日常 1.1.0 安装**（`C:\Users\DELL\.codex\plugins\cache\claudetocodex-dev\claudetocodex\1.1.0`）。即时恢复：从 git tag `v1.1.0` `git archive` 原位恢复，19/19 文件与 tag 一致、plugin.json=1.1.0、hooks.json 哈希一致、在跑会话 hook 路径重新有效；桥数据根与 pair 全程未动。残余：插件注册已注销（PO 需按 INSTALL.md 两条命令在常规终端重装 1.1.0）、杂散树待确认后删除、隔离候选安装暂停待 SM 重新授权（重做流程：显式设置并回显 `CODEX_HOME`，installedPath 前缀验证通过才继续）。教训：任何宿主状态命令前先回显生效环境；环境赋值失败必须中断而非续行。
- 待办：PO 重装日常 1.1.0（注册恢复）→ SM 重新授权后重做隔离候选安装 → PO 亲自 host trust 与端到端真实验收（SMOKE 4c R1–R6 格）→ PO DoD 检视。