...
- **2026-09-13 W7 隔离宿主安装事故与恢复（S05-SM-HOST-INSTALL-09 期间，已回报 SM：9f5ef8be；SM 事故指令：S05-INCIDENT-10）。**
  ...
  - 剩余风险与待决（2026-09-13 更新，S05-RESTORE-11 / S05-CLEANUP-12）：插件注册已恢复——**PO 授权 SM 代执行**重装并只读复核（installed、enabled、1.1.0、manifest 19 文件、hooks 在位）；杂散树已清理——**PO 授权 SM** 先核验解析路径与日常 1.1.0 存在，再移入项目 `.trash` 安全边界逐文件清理，`C:\Users\DELL\plugins` 已不存在，日常 1.1.0 仍 installed、enabled；隔离候选安装**继续暂停，待 PO 确认/授权**，重做防护不变。在获得授权前，Developer **禁止**任何改变 codex plugin/marketplace 状态、删除目录或重启宿主的操作（S05-INCIDENT-10 冻结令）。
...
- 待办：~~PO 亲自重装日常 1.1.0（注册恢复）~~（已由 PO 授权 SM 完成，S05-RESTORE-11）→ ~~PO 确认后清理杂散树~~（已由 PO 授权 SM 完成，S05-CLEANUP-12）→ PO/SM 授权后按上述防护重做隔离候选安装 → PO 亲自 host trust 与端到端真实验收（SMOKE 4c R1–R6 格）→ PO DoD 检视。