...
test('Stop creates a continuation only for new messages, including after an earlier continuation', (t) => {
  const { store, pairA } = setup(t, { withB: false });
  const first = incoming(store, pairA, 'Arrived during final generation.');
...
test('normal prompts and other sessions are not blocked or mistaken for bridge wakes', (t) => {
  const { store, pairA } = setup(t, { withB: false });
  const message = incoming(store, pairA);
  assert.deepEqual(handleHook(store, hookEvent('PostToolUse', { session_id: '55555555-5555-4555-8555-555555555555' })), {});
  ...
  // A plain prompt is a delivery opportunity, never a blocked or mistaken wake.
  const delivered = handleHook(store, hookEvent('UserPromptSubmit', { prompt: 'Continue my work.' }));
  assert.ok(delivered.hookSpecificOutput.additionalContext.includes(message.body));
  // A wake quoted inside other text is ordinary input, not a wake.
  assert.deepEqual(handleHook(store, hookEvent('UserPromptSubmit', { prompt: `Quoted example: ${wakeText(pairA.id, message.id)}` })), {});
});
...
test('concurrent hook processes claim a pending message only once', async (t) => {
  ...
  const invoke = () => new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [cli, 'hook'], { env, windowsHide: true, timeout: 5000 });
    let output = '';
    let errors = '';
...
test('a changed Claude endpoint identity is rejected before any pipe write', (t) => {
  ...
  writeFileSync(epA, JSON.stringify({ sessionId: '55555555-5555-4555-8555-555555555555', socket: '\\\\.\\pipe\\x', tokenProtected: 'opaque' }));
  const env = { ...process.env, CTC_BRIDGE_DIR: root, CODEX_THREAD_ID: codexId };
  delete env.CLAUDE_CODE_SESSION_ID;
