...
if (-not $sessionId -or $sessionId -notmatch '^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$' -or
    ...
    throw 'Run this command from the selected Claude Code session tool, with cross-session messaging available.'
}
if (-not $socket.StartsWith('\\.\pipe\')) {
    throw 'This bridge expects a native Windows named pipe.'
}
