...
Windows-only；短文本（trim 后 1..2000 字符）；串行投递（一个 Codex 会话可同时连接多个 Claude 会话、按名称往来，恰一目标时沿用免 `--name` 的老命令形态）；回执恒 `unverified`；失败不自动重试；Claude 会话重启后旧端点失效，重新连接即可（同身份沿用并刷新端点；弃用的旧配对用 `retire --pairId` 显式退役，不静默替换）；CLI 版本升级后的契约漂移未验证。
