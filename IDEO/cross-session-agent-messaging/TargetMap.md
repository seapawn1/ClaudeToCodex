...
使用已安装的 Node.js 24.14.0，利用内置 SQLite 保存待收消息和注入记录，无需安装外部依赖。

提供 `pair`、`send`、`reply`、`status` 操作。消息包含唯一 ID、发送会话、接收会话、所属交流、回复对象和正文；调用采用结构化参数或文件输入。

代码放在本目标目录的 `prototype/`。运行数据存放在用户临时目录，以支持不同 worktree 访问同一配对；令牌限制为当前操作系统用户可读，不进入 Git。

#### 显式配对两个原会话

通过两端的 `SessionStart` Hook 登记真实会话标识；Claude 同时登记本会话导出的管道地址与令牌。

首次配置时明确选择一对会话。配对之后按精确标识投递，其他会话启动不能覆盖当前配对。原型暂不自动选择“最新会话”或自动重新配对。
...
上述入口共用去重记录。已由工作中 Hook 注入的消息，其残留唤醒信号不得再次交付正文；能否在 `UserPromptSubmit` 阶段安全抑制该重复信号，列为首轮适配实验的必查项。
