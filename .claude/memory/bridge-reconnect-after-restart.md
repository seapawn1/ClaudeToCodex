...
ClaudeToCodex 1.2 supports multiple pairs in one per-Codex bridge data root and automatically selects/reuses that root by Codex thread. Apply these lifecycle rules:
...
5. One bridge data root serves one Codex original session. In 1.2+, connect/status/hooks resolve the same per-Codex root automatically; explicit `CTC_BRIDGE_DIR` is only an isolation override. Do not move, merge, or archive the whole root merely to recover one Claude target.
