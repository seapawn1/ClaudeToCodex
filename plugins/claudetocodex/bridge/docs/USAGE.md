...
本产品是运行于本机（Windows 与 WSL2/Linux，同一 OS 用户）的 Codex ↔ Claude Code 跨会话双向消息桥：让**已在运行的原始会话**互发短消息、收到回复、继续对话。一个 Codex 原始会话可同时与多个 Claude 原始会话保持配对并按名称往来。以下四类任务按顺序覆盖安装、配置、发起通信与回复。

## 1. 安装

前置：Windows 或 WSL2 Linux（同一 OS 用户运行两侧会话）；Node.js ≥18.3；参与通信的 codex CLI（已验证 `0.153.4` 与 `0.154.0`）与 claude CLI（已验证 `2.1.263`、`2.1.268`，Linux 侧 `2.1.275`）在 PATH 上。
...
数据目录（**自动按 Codex 会话选择与复用，无需手写路径或 ID**）。数据基址按平台：Windows 为 `%LOCALAPPDATA%\ClaudeToCodex`，Linux 为 `~/.local/share/ClaudeToCodex`（`XDG_DATA_HOME` 可覆盖；空串视同未设置）。解析次序：

1. 显式 `CTC_BRIDGE_DIR`——测试与隔离覆盖，最高优先；设置后完全绕过会话索引（不读不写）。
2. 会话索引命中：数据基址下 `bridge-roots/` 目录（每会话一个 JSON 文件；`CTC_ROOTS_DIR` 可覆盖其位置）记录 codexThreadId→数据根；同一 thread 完全退出后 resume 自动复用原根。索引只在 connect 成功后登记，**只增不改绑**；多会话并发首连各写各的文件，互不丢失。
3. 默认根 数据基址/`bridge`：未被任何 Codex 占用、或已属当前会话（存量沿用，零数据迁移）时采用。
4. 默认根被其他 Codex 占用（含仅剩退役存档）时，connect 自动为当前会话启用新根 数据基址/`bridge-threads/<threadId>` 并登记索引；不改绑、不迁移、不动旧根数据。
...
- 重叠来信：多个目标相近时间回复时各自落槽、逐事件注入（唤醒顺序决定注入顺序，无唤醒时按最旧兜底）；**同一轮内**的重复唤醒（如项目级与插件级双 hook 注册对同一条消息各处理一次）被放行为空操作并留 `wake-same-turn-noop` 事件，**跨轮**的真实重复唤醒仍被抑制；投递不因中途退役而丢失，也不注入未知来源。
- 到达可读性（Sprint 03 起）：Claude→Codex 的排队唤醒文本为多行可读消息——头行 `[Source: bridge message | <时间>]`、空行、完整正文、空行、独立 `[CTC-WAKE <pairId> <messageId>]` 标记行。可执行回复入口由 hook 注入层提供（不进入排队文本），正文已在排队文本中时注入层不再重复注入正文；排队中是正常状态，不是故障。正文内容不会被解析或改写，正文中出现的 marker 样式文本行不影响投递。在途的旧单行 `[CTC-WAKE ...]` 唤醒仍可解析投递。
...
- 模型生成中/工具执行中的所有宿主异常注入形态；已验证边界以 Sprint Review 与测试记录为准，不用模型自述替代原始事件。
- 长文本与重复投递的组合；持久台账、送达回执、失败重试与事务性投递。
- 来源防伪、令牌生命周期与多用户安全。
- 原生 Linux（非 WSL2）、macOS 与其他平台；Windows 与 Linux 之间的**跨侧**通信（桥只在同一 OS 用户内工作）；`priority=now/later` 不在产品路径（普通消息固定 `priority=next`）。Windows 侧投递使用命名管道＋同用户 DPAPI 保护 token；Linux 侧使用 Unix domain socket＋发送时现读会话注册表 peer key（桥数据根零秘密落盘）。
